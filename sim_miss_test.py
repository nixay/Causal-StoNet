...
from data import SimData_Missing_test
...
parser = argparse.ArgumentParser(description='Run simulaiton for Causal StoNet')
# Basic Setting
# simulation setting
...
parser.add_argument('--para_lr_decay', default=1, type=float, help='decay factor for para_lr')
parser.add_argument('--impute_lr_decay', default=1, type=float, help='decay factor for impute_lr')

# Parameters for Sparsity
parser.add_argument('--num_run', default=3, type=int, help='Number of different initialization used to train the model')
...
def main():
    ...
    data = SimData_Missing_test(data_seed)
    ...
    base_path = os.path.join('.', 'sim_miss_test', 'result', str(data_seed))
    ...
    for prune_seed in range(num_seed):
        ...
        with torch.no_grad():
            ...
            for y, treat, x, *rest in test_data:
