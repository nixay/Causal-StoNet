...
class SimData_2(Dataset):
    ...
    def __init__(self, seed, data_size):
        ...
        for i in range(self.data_size):
            ...
            h1 = np.tanh(1*x_temp[0]-2*x_temp[1])
            h2 = np.tanh(1*x_temp[2]-3*x_temp[0])
            h3 = np.tanh(2*x_temp[2]+4*x_temp[3])
            h4 = np.tanh(2*x_temp[4])

            # generate treatment
            score = np.tanh(2*h1+3*h2-1*h3) + np.random.normal(0, 1)
            ...
            y_temp = np.tanh(3 * np.tanh(-3 * np.tanh(-4 * h1 + 2 * h3) + 5 * treat_temp) + 1 * np.tanh(
                1 * h2 - 2 * h4)) + np.random.normal(0, 1)
...
def optimization(net, train_data, val_data, epochs, para_lrs):
    ...
    for epoch in range(epochs):
        ...
        for name, para in net.named_parameters():
            para_path[name][str(epoch)] = para.data.cpu().numpy()
            para_gamma_path[name][str(epoch)] = (para.abs() > threshold).data.cpu().numpy()
...
def main():
    ...
    data = SimData_2(data_seed, train_size + val_size)
