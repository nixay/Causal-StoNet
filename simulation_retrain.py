...
def main():
    ...
    # path to save the result
    PATH = os.path.join(base_path, 'retrain')
    para_path = args.model_para_path
    ...
    num_selection = np.sum(var_gamma_train[str(training_epochs-1)])
    ...
    results = {}
    results['Overall_train_loss'] = train_loss
    results['Overall_val_loss'] = val_loss
    if regression_flag is False:
        results['Overall_train_accuracy'] = train_accuracy
        results['Overall_val_accuracy'] = val_accuracy
    results['Overall_BIC'] = BIC
    results['Overall_non_zero_connections'] = int(dim)
    results['Overall_selected_variables'] = int(num_selection)

    result_file = open(os.path.join(PATH, 'result.json'), "w")
    json.dump(results, result_file, indent="")
    result_file.close()
